...
class BirthDateAnonymizerOperator(Operator):
    ...
    def operate(self, text: str, params: Dict = None) -> str:
        ...
        if date_container.year is None:
            return text
    ...
    def calculate_estimated_age(date_mention: DateMention) -> int:
        ...
        if  (date_mention.year.text.isdigit()):
            ...
            int_year = date.today().year-50

        if date_mention.month is not None and date_mention.month.text.isdigit():
            int_month = int(date_mention.month.text)
        else:
            int_month = date.today().month
            
        if date_mention.day is not None and date_mention.day.text.isdigit():
            int_day = int(date_mention.day.text)
        else:
            int_day = date.today().day
            
        birth_date = date(int_year, int_month, int_day)

    ...
    def handle(self, entities: List[RecognizerResult], doc: Doc) -> List[RecognizerResult]:
        ...
        if self.is_conflict_between(["ID", "DATE"], group_categories):
            if len(entities) > 2:
                ...
                id_entity = entities[0] if ENTITY_TYPE_TO_CATEGORY[entities[0].entity_type] == "ID" or len(entities)==1  else entities[1]
    ...
    def handle(self, entities: List[RecognizerResult], doc: Doc) -> List[RecognizerResult]:
        ...
        for entity in entities:
            if entity not in entity_boundaries_to_entity[longest_entity_start, longest_entity_end]:
                # if the context is pathology and longest entity is ID just prefer the longest entity
                if VARIABLES['context'] == 'pathology' and longest_entity.entity_type == 'ID':
                    return self.prefer_longest_entity_resolver(entities, doc)

...
class FilterEntities:
    ...
    def __call__(recognized_entities: List[RecognizerResult], doc: Doc) -> List[RecognizerResult]:
        ...
        # filter out NAME entities that contains at least one number 
        filtered_entities = list(
             filter(lambda entity: ENTITY_TYPE_TO_CATEGORY[entity.entity_type] != "NAME"
                                   or FilterEntities.NUMBER_REGEX.search(doc.text[entity.start:entity.end]) is None , filtered_entities))
